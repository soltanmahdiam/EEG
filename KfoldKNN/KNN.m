...
for fi = 1:7
    ...
    for fol = 1:k
        ...
        Mdl = fitcknn([Pop(trnIndxs).features]',[Pop(trnIndxs).label]','NumNeighbors',25,...
        'NSMethod','exhaustive','Distance','cityblock',...
        'Standardize',0);
        
        KNNdata.KNNOut = predict(Mdl,[Pop(tstIndxs).features]');
        

        % Calc Error
        
        confmat = confusionmat(KNNdata.KNNOut,[Pop(tstIndxs).label]'); % where response is the last column in the dataset representing a class
        TP = confmat(2, 2);
        TN = confmat(1, 1);
        FP = confmat(1, 2);
        FN = confmat(2, 1);
...
% writetable(Meters,'KNN_Times.xlsx')
TableWriter(T2,'KNN_Times.xlsx')
